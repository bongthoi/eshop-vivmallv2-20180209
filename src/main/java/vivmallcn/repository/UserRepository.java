...
public class UserRepository extends CustomizeRepository<User, String> {
	...
	public Long getNewUserSize() {
		...
		Long count = this.getJdbcTemplate().queryForObject("select count(*) from " +this.getTable().getName() + " where enabled=0", Long.class);
		return count;
	}

	public Page<User> findNewUsersByIsEnabled(int enabled, Pageable page) {
		// TODO Auto-generated method stub
		Page<User> users=this.findQuery(" where enabled="+enabled,page);
		return users;
	}
}